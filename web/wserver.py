...
sabnzbd_client = SabnzbdClient(
    host="http://localhost",
    api_key="admin",
    port="",  # Changed port to Require port
)
SERVICES = {
    "nzb": {"url": "http://localhost:/"},  # Changed port to Require port
...
async def lifespan(app: FastAPI):
    ...
    aria2 = Aria2HttpClient("http://localhost:/jsonrpc")  # Changed port to Require port
